...
function main() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (!filePath) {
            const initialChoice = yield select({
            ...
            while (!isValidFile) {
                filePath = yield (initialChoice.method === 'explore' ? chooseItems({ currentPath, includeFiles: true }) : input({
                ...
                try {
                    ...
                    if (validateOutputJson(fileContent)) {
                        ...
                        console.error('❌ The selected file is not valid. Please select a correct output.json file.');
                    }
                }
                catch (_error) {
                    console.log('Failed to read or parse the file. Please select a correct output.json file.');
                }
            }
        }
        else {
        ...
        if (!reportPath) {
            ...
            const reportFolderChoice = yield select({
            ...
            reportPath = yield (reportFolderChoice.method === 'explore' ? chooseItems({ currentPath }) : input({
                message: 'Please enter the file path:',
            }));
        }
        ...
        const runInDebugMode = logLevel === 'debug' || (!isCliMode && (yield confirm({
        ...
        if (!isLocalDev) {
            ...
            const copyReportSpinner = ora(`Copying report to ${reporterPath}...\n`).start();
            try {
                if (!existsSync(reporterPath)) {
                    mkdirSync(reporterPath, { recursive: true });
                }
                copyDirectory(join(visualReporterProjectRoot, 'build', 'client'), reporterPath);
                copyReportSpinner.succeed(`Build output copied successfully to "${reporterPath}".`);
            }
            catch (error) {
        ...
        if (!isCliMode) {
            ...
            const startServer = yield confirm({
